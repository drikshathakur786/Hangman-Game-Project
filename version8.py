...
ACHIEVEMENT_SCORE = 5
ACHIEVEMENT_CORRECT_GUESSES = 10
...
class HangmanGame:
    ...
    def show_achievement(self):
        if self.score >= ACHIEVEMENT_SCORE:
            messagebox.showinfo("Achievement Unlocked!", "You've reached a score of {}! Keep it up!".format(ACHIEVEMENT_SCORE))
        if self.correct_guesses >= ACHIEVEMENT_CORRECT_GUESSES:
            messagebox.showinfo("Achievement Unlocked!", "You've made {} correct guesses! Great job!".format(ACHIEVEMENT_CORRECT_GUESSES))

    # Select a new difficulty level
    def select_level(self):
        messagebox.showinfo("Select Level", "Choose a new difficulty level.")
        self.reset_game()
    ...
    def create_keyboard(self):
        ...
        for letter in alphabet:
            button = tk.Button(keyboard_frame, text=letter, command=lambda l=letter: self.make_guess(l))
            button.grid(row=(ord(letter) - 97) // 9, column=(ord(letter) - 97) % 9)
...
def select_level():
    ...
    start_button = tk.Button(level_window, text="Start Game", command=lambda: [level_window.destroy(), start_game(var.get())], font=("Helvetica", 12))
